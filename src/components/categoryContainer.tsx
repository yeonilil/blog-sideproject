import { categoriesSearch, getCategories } from "@/pages/api/categories";
    ...
    <div className="flex flex-col w-[400px] md:w-[600px] xl:w-[700px] mt-[20px] ">
      ...
      <div className="flex gap-[4px] mb-[10px]">
        {categories.map((category: Category, index: number) => (
          <p
            key={index}
            className="w-fit text-[13px] text-black-400  inline-block transform rounded-[8px] bg-blue-200 px-[8px] py-[4px] transition hover:scale-105"
          >
            {category.name}
          </p>
        ))}
